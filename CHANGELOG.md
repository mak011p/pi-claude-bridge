...
- **Fix: diagnostic logging cannot abort a model request** — logs default to Pi's temporary state directory, respect diagnostic/debug path overrides, and tolerate filesystem failures. Empty history imports start fresh instead of verifying and resuming a session file that was never created.
